...
$.extend(Keypad.prototype, {
	...
	_selectValue: function(inst, value) {
		var posObj = this.insertValue(inst._input[0], value);// ���ص�ǰ�����ַ��λ�õ���Ϣ
		this._setValue(inst, inst._input.val(),posObj);
	...
	insertValue: function(input, value) {
		...
		*/
		var posObj = {currPos:pos,startPos:range[0]};
		this._setCursorPos(posObj);
		return posObj;
	},
	/*
	  ���ù�����ı��е�λ��
	   @param  field  (element) the target text field
	   @param  posObj (object) �������λ�õĶ���
	 */
	_setCursorPos:function(input, posObj){
		input = (input.jquery ? input : $(input));
		var field = input[0];
		if (field.setSelectionRange) { // Mozilla
			if (input.css('display') != 'none') {
				field.setSelectionRange(posObj.currPos, posObj.currPos);
			}
		}
		else if (field.createTextRange) { // IE
			var range = field.createTextRange();
			range.move('character', posObj.currPos);
			range.select();
		}
	},
	...
	/* Set the text field to the selected value,
	   ...
	   @param  posObj  (object) ���λ�ö��� */
	_setValue: function(inst, value, posObj) {
